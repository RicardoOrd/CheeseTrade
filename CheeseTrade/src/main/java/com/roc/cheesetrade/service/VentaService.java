...
public class VentaService {
    ...
    private boolean realizarVentaCompleta(List<Inv> carrito, int idUsuario, VentaDAO ventaDAO, DetalleVentaDAO detalleDAO, InvDAO invDAO) {
        ...
        try {
            ...
            // Insertar la venta
            int idVenta = ventaDAO.insertarVenta(con, new java.sql.Date(System.currentTimeMillis()), total, idUsuario);
            if (idVenta == -1) {
                con.rollback();
                return false;
            }
            ...
            int regHistorial = ventaDAO.registrarHistorialCompra(con, idVenta, total, "efectivo", "pagado");
            if (regHistorial == 0) {
                con.rollback();
                return false;
            }
            ...
            for (Inv prod : carrito) {
                int detalle = detalleDAO.insertarDetalleVenta(con, idVenta, prod.getIdProducto(), prod.getStock(), prod.getPrecio());
                int actualizado = invDAO.descontarStock(con, prod.getIdProducto(), prod.getStock());
